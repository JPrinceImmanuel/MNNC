...
def rank_clusters_by_density(labels, densities, n_clusters):
    """Rank clusters based on average density."""
    avg_densities = []
    for cluster_id in range(n_clusters):
        avg_density = densities[labels == cluster_id].mean()
        avg_densities.append((cluster_id, avg_density))
    avg_densities.sort(key=lambda x: x[1])  # Sort by density (ascending)
    ranked_clusters = [cluster_id for cluster_id, _ in avg_densities]
...
features, density = extract_features(points_normalized.cpu().numpy())
labels = cluster_points(features, args.n_clusters)

# Rank clusters by density
ranked_clusters = rank_clusters_by_density(labels, density, args.n_clusters)
